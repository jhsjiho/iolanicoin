Iolanicoin Core version *0.15.1* is now available from:

  <https://download.iolanicoin.org/iolanicoin-0.15.1/>
...
A number of changes to the way Iolanicoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 mIOC per kB (0.00001 IOC). This is to prepare for dropping the minimum transaction fee to 0.1 mIOC per kB (0.0001 IOC) in the next release.
